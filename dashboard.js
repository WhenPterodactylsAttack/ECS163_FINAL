...
function drawSankey() {
  ...
  const graph = sankey().nodeWidth(20).nodePadding(10).extent([[1, 40], [800 - 1, 500 - 1]])(sankeyData);
  d3.select("#sankey").html("");
  const svg = d3.select("#sankey").append("svg").attr("width", 800).attr("height", 540);
  ...
  svg.append("text")
    ...
    .attr("font-size", "16px")
    .attr("font-weight", "bold")
    .text("Penguin Attributes Sankey Diagram");
